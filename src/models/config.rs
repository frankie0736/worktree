...
mod tests {
    ...
    fn test_config_minimal() {
        let yaml = "agent_command: claude -p\n";
        let config = WtConfig::from_str(yaml).unwrap();

        assert_eq!(config.agent_command, "claude -p");
...
"#;
    ...
    #[test]
    fn test_config_missing_agent_command() {
        let yaml = "tmux_session: wt\n";
    ...
    fn test_config_invalid_yaml() {
        let yaml = "agent_command: [invalid";
        let result = WtConfig::from_str(yaml);
        assert!(result.is_err());
    }
...
"#;
    ...
    fn test_config_serialize() {
        ...
        assert!(yaml.contains("agent_command: test"));
